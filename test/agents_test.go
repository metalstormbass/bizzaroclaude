...
// to the per-repo agents directory during `bizzaroclaude init`.
...
func TestAgentDefinitionMerging(t *testing.T) {
	...
	if workerDef, ok := defMap["worker"]; ok {
		...
		if !strings.Contains(workerDef.Content, "bizzaroclaude agent complete") {
...
// TestAgentsListCommand verifies that `bizzaroclaude agents list` shows available definitions.
...
// TestAgentsResetCommand verifies that `bizzaroclaude agents reset` restores defaults.
...
// TestAgentsSpawnCommand verifies that `bizzaroclaude agents spawn` creates an agent
