...
func TestCopyHooksConfig(t *testing.T) {
	...
	repoName := "test-repo"
	repoPath := d.paths.RepoDir(repoName)
	if err := os.MkdirAll(filepath.Join(repoPath, ".bizzaroclaude"), 0755); err != nil {
		t.Fatalf("Failed to create .bizzaroclaude dir: %v", err)
	}

	// Create hooks.json
	hooksContent := `{"hooks": [{"event": "test", "command": "echo test"}]}`
	hooksPath := filepath.Join(repoPath, ".bizzaroclaude", "hooks.json")
...
`
	...
	t.Run("includes spawn instructions", func(t *testing.T) {
		...
		if !strings.Contains(msgContent.Body, "bizzaroclaude agents spawn") {
