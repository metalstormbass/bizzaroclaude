...
func DaemonNotRunning() *CLIError {
	return &CLIError{
		...
		Suggestion: "bizzaroclaude daemon start",
	}
}
...
func DaemonCommunicationFailed(operation string, cause error) *CLIError {
	return &CLIError{
		...
		Cause:      cause,
		Suggestion: "bizzaroclaude daemon status",
	}
}
...
func NotInRepo() *CLIError {
	return &CLIError{
		...
		Suggestion: "bizzaroclaude repo init <github-url> to track a repository, or use --repo flag",
	}
}
...
func AgentNotFound(agentType, name, repo string) *CLIError {
	return &CLIError{
		...
		Suggestion: fmt.Sprintf("bizzaroclaude worker list --repo %s", repo),
	}
}
...
func NotInAgentContext() *CLIError {
	return &CLIError{
		Category:   CategoryConfig,
		Message:    "not in a bizzaroclaude agent directory",
...
func NoRepositoriesFound() *CLIError {
	return &CLIError{
		...
		Suggestion: "bizzaroclaude repo init <github-url>",
	}
}
...
func NoWorkersFound(repo string) *CLIError {
	return &CLIError{
		...
		Suggestion: fmt.Sprintf("bizzaroclaude worker create \"<task>\" --repo %s", repo),
	}
}
...
func NoWorkspacesFound(repo string) *CLIError {
	return &CLIError{
		...
		Suggestion: fmt.Sprintf("bizzaroclaude workspace add <name> --repo %s", repo),
	}
}
...
func WorkspaceNotFound(name, repo string) *CLIError {
	return &CLIError{
		...
		Suggestion: fmt.Sprintf("bizzaroclaude workspace list --repo %s", repo),
	}
}
