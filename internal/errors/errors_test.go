...
func TestFormat_CLIError(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			err:      New(CategoryConnection, "daemon offline").WithSuggestion("bizzaroclaude daemon start"),
			contains: []string{"daemon offline", "Try:", "bizzaroclaude daemon start"},
		},
	}
...
func TestDaemonNotRunning(t *testing.T) {
	...
	if !strings.Contains(formatted, "bizzaroclaude daemon start") {
...
func TestNotInRepo(t *testing.T) {
	...
	if !strings.Contains(formatted, "bizzaroclaude repo init") {
...
func TestAgentNotFound(t *testing.T) {
	...
	if !strings.Contains(formatted, "bizzaroclaude worker list") {
...
func TestWorktreeCreationFailed_SpecificSuggestions(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			wantContains: []string{"work/nice-owl", "bizzaroclaude cleanup", "git branch -D work/nice-owl"},
		},
		{
			name:         "generic branch already exists",
			causeMsg:     "branch already exists",
			wantContains: []string{"bizzaroclaude cleanup", "previous run"},
		},
		{
			name:         "worktree path exists",
			causeMsg:     "path already exists",
			wantContains: []string{"bizzaroclaude cleanup", "worktree directory"},
		},
		{
			name:         "is a worktree error",
			causeMsg:     "is a worktree",
			wantContains: []string{"bizzaroclaude cleanup"},
		},
		{
		...
		},
		{
			...
			wantContains: []string{"bizzaroclaude cleanup", "another worktree"},
		},
		{
			name:            "generic error falls back to default",
			causeMsg:        "some random error",
			wantContains:    []string{"disk space", "repository state"},
			wantNotContains: []string{"bizzaroclaude cleanup", "git branch"},
		},
		{
...
func TestNoWorkersFound(t *testing.T) {
	...
	if !strings.Contains(formatted, "bizzaroclaude worker create") {
...
func TestNoWorkspacesFound(t *testing.T) {
	...
	if !strings.Contains(formatted, "bizzaroclaude workspace add") {
...
func TestWorkspaceNotFound(t *testing.T) {
	...
	if !strings.Contains(formatted, "bizzaroclaude workspace list") {
