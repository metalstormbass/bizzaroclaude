...
func TestGetDefaultPromptContent(t *testing.T) {
	...
	if !strings.Contains(supervisorPrompt, "bizzaroclaude message send") {
...
func TestLoadCustomPrompt(t *testing.T) {
	...
	tmpDir, err := os.MkdirTemp("", "bizzaroclaude-prompts-test-*")
	if err != nil {
		t.Fatalf("failed to create temp dir: %v", err)
	}
	defer os.RemoveAll(tmpDir)

	// Create .bizzaroclaude directory
	bizzaroclaudeDir := filepath.Join(tmpDir, ".bizzaroclaude")
	if err := os.MkdirAll(bizzaroclaudeDir, 0755); err != nil {
		t.Fatalf("failed to create .bizzaroclaude dir: %v", err)
	}
	...
	t.Run("with custom supervisor prompt", func(t *testing.T) {
		...
		promptPath := filepath.Join(bizzaroclaudeDir, "SUPERVISOR.md")
	...
	t.Run("with custom worker prompt", func(t *testing.T) {
		...
		promptPath := filepath.Join(bizzaroclaudeDir, "WORKER.md")
		if err := os.WriteFile(promptPath, []byte(customContent), 0644); err != nil {
			t.Fatalf("failed to write custom prompt: %v", err)
		}
	...
	t.Run("with custom merge-queue prompt", func(t *testing.T) {
		...
		promptPath := filepath.Join(bizzaroclaudeDir, "MERGE-QUEUE.md")
		if err := os.WriteFile(promptPath, []byte(customContent), 0644); err != nil {
			t.Fatalf("failed to write custom prompt: %v", err)
		}
	...
	t.Run("with custom workspace prompt", func(t *testing.T) {
		...
		promptPath := filepath.Join(bizzaroclaudeDir, "WORKSPACE.md")
		if err := os.WriteFile(promptPath, []byte(customContent), 0644); err != nil {
			t.Fatalf("failed to write custom prompt: %v", err)
		}
	...
	t.Run("with custom review prompt", func(t *testing.T) {
		...
		promptPath := filepath.Join(bizzaroclaudeDir, "REVIEW.md")
		if err := os.WriteFile(promptPath, []byte(customContent), 0644); err != nil {
			t.Fatalf("failed to write custom prompt: %v", err)
		}
...
func TestGetSlashCommandsPromptContainsCLICommands(t *testing.T) {
	...
	statusCommands := []struct {
		...
		{"bizzaroclaude daemon status", "/status should include daemon status check"},
	...
	workersCommands := []struct {
		...
		{"bizzaroclaude worker list", "/workers should include worker list command"},
	}
	...
	messagesCommands := []struct {
		...
		{"bizzaroclaude message list", "/messages should include list command"},
	}
