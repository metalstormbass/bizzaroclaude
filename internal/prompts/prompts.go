...
func GetDefaultPrompt(agentType state.AgentType) string {
	...
	case state.AgentTypeWorker, state.AgentTypeMergeQueue, state.AgentTypeReview:
		...
		// from ~/.bizzaroclaude/repos/<repo>/agents/ or <repo>/.bizzaroclaude/agents/
		return ""
	default:
		return ""
	}
...
// LoadCustomPrompt loads a custom prompt from the repository's .bizzaroclaude directory.
// Returns empty string if the file doesn't exist.
//
// Deprecated: This function is deprecated. Use the configurable agent system instead:
// - Agent definitions: <repo>/.bizzaroclaude/agents/<agent-name>.md
// - Local overrides: ~/.bizzaroclaude/repos/<repo>/agents/<agent-name>.md
...
func LoadCustomPrompt(repoPath string, agentType state.AgentType) (string, error) {
	...
	promptPath := filepath.Join(repoPath, ".bizzaroclaude", filename)
...
**IMPORTANT**: This repository is configured to track only PRs where you (or the bizzaroclaude system) are the author.

When listing and monitoring PRs, use:
` + "```bash" + `
gh pr list --author @me --label bizzaroclaude
` + "```" + `

Do NOT process or attempt to merge PRs authored by others. Focus only on PRs created by bizzaroclaude workers.`

	case "assigned":
		return `## PR Tracking Mode: Assigned Only

**IMPORTANT**: This repository is configured to track only PRs where you (or the bizzaroclaude system) are assigned.

When listing and monitoring PRs, use:
` + "```bash" + `
gh pr list --assignee @me --label bizzaroclaude
` + "```" + `

Do NOT process or attempt to merge PRs unless they are assigned to you. Focus only on PRs explicitly assigned to bizzaroclaude.`

	default: // "all"
		return `## PR Tracking Mode: All PRs

This repository is configured to track all PRs with the bizzaroclaude label.

When listing and monitoring PRs, use:
` + "```bash" + `
gh pr list --label bizzaroclaude
` + "```" + `

Monitor and process all bizzaroclaude-labeled PRs regardless of author or assignee.`
	}
}
